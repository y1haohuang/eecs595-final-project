...
from config import EMBEDDINGS_EXPORT_PATH, TMP_DIR, ENGLISH_TEST_TARGET_WORDS, TEST_DATA_RESULTS_DIR, \
    ...
    ENGLISH_TEST_GOLD_TASK_1, ENGLISH_TEST_CORPUS_1, ENGLISH_TEST_CORPUS_2, GERMAN_TEST_CORPUS_1, \
    GERMAN_TEST_CORPUS_2, LATIN_TEST_CORPUS_1, LATIN_TEST_CORPUS_2, SWEDISH_TEST_CORPUS_1, SWEDISH_TEST_CORPUS_2, \
    ISLAND_DIR
from data.post_eval_data.scoring_program.evaluation_official import spearman_official, accuracy_official
from sense_comparator import load_transform_matrix, compare_sense, compare_sense_island
from embeddings.embeddings_generator import load_test_data

island_size = 10
island_num = 9
temporal_referencing = False
NO_SWEDISH = True
...
def main():
    ...
    # #
    acc, rho = run_english_default(task_1_dir, task_2_dir, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island, emb_type, emb_dim, window, iter, mean_centering, unit_vectors)
    acc_list.append(acc)
    rho_list.append(rho)

    acc, rho = run_german_default(task_1_dir, task_2_dir, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island, emb_type, emb_dim, window, iter, mean_centering, unit_vectors)
    acc_list.append(acc)
    rho_list.append(rho)

    acc, rho = run_latin_default(task_1_dir, task_2_dir, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island, emb_type, emb_dim, window, iter, mean_centering, unit_vectors)
    acc_list.append(acc)
    rho_list.append(rho)

    if not NO_SWEDISH:
        acc, rho = run_swedish_default(task_1_dir, task_2_dir, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island, emb_type, emb_dim, window, iter, mean_centering, unit_vectors)
        acc_list.append(acc)
        rho_list.append(rho)
    ...
    if NO_SWEDISH:
        print('Type' + '\t' + 'avg acc/rank' + '\t' + 'english' + '\t' + 'german' + '\t' + 'latin'+ '\t' # + 'swedish' + '\t'
              + 'reverse emb'
        ...
        print(
            'Type' + '\t' + 'avg acc/rank' + '\t' + 'english' + '\t' + 'german' + '\t' + 'latin' + '\t'  + 'swedish' + '\t'
            + 'reverse emb'
            + '\t' + 'emb_type' + '\t' + 'emb_dim' + '\t' + 'window' + '\t' + 'iter' + '\t' + 'use bin thld' + '\t' + 'use nearest neigh')
        print("Binary overview" + '\t' + str(acc_avg) +
              '\t' + str(acc_list[0]) + '\t' + str(acc_list[1]) + '\t' + str(
            acc_list[2]) + '\t'  + str(acc_list[3]) + '\t'
              + str(reverse_emb) + '\t' + emb_type + '\t' + str(
            emb_dim) + '\t' + str(window) + '\t' + str(iter)
              + '\t' + str(use_bin_thld) + '\t' + str(use_nearest_neigbh))

        print('Rank overview' + '\t' + str(rho_avg) +
              '\t' + str(rho_list[0]) + '\t' + str(rho_list[1]) + '\t' + str(
            rho_list[2]) + '\t'  + str(rho_list[3]) + '\t'
              + str(reverse_emb) + '\t' + emb_type + '\t' + str(
            emb_dim) + '\t' + str(window) + '\t' + str(iter))
...
def run_swedish_default(task_1_dir, task_2_dir, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island,
    ...
    if temporal_referencing:
        corp1_emb_file = emb_type + '.skipgram.swedish_corpus.' + str(
            emb_dim) + '_window-' + str(window) + '_iter-' + str(
            iter) + '_min-count-5.vec'
        corp_1_emb = os.path.join(EMBEDDINGS_EXPORT_PATH, 'swedish_corpus_temporal',
                                  corp1_emb_file)
        corp_2_emb = corp_1_emb

    target_words = SWEDISH_TEST_TARGET_WORDS
    rho, acc, bin_thld, min_neighb_cnt = compare(SWEDISH_TEST_CORPUS_1, SWEDISH_TEST_CORPUS_2, corp_1_emb, corp_2_emb, target_words, SWEDISH_TEST_GOLD_TASK_1,
                                                 SWEDISH_TEST_GOLD_TASK_2, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island,
                                                 mean_centering, unit_vectors, save_file_ranks=save_file,
                                                 save_file_binary=save_file_binary, one_minus=True, name="swedish")
...
def run_latin_default(task_1_dir, task_2_dir, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island,
                      emb_type, emb_dim, window, iter, mean_centering, unit_vectors):
    print('-' * 70)
    print('Latin')
    ...
    if temporal_referencing:
        corp1_emb_file = emb_type + '.skipgram.latin_corpus.' + str(
            emb_dim) + '_window-' + str(window) + '_iter-' + str(
            iter) + '_min-count-5.vec'
        corp_1_emb = os.path.join(EMBEDDINGS_EXPORT_PATH, 'latin_corpus_temporal',
                                  corp1_emb_file)
        corp_2_emb = corp_1_emb

    target_words = LATIN_TEST_TARGET_WORDS
    rho, acc, bin_thld, min_neighb_cnt = compare(LATIN_TEST_CORPUS_1, LATIN_TEST_CORPUS_2, corp_1_emb, corp_2_emb, target_words, LATIN_TEST_GOLD_TASK_1,
                                                 LATIN_TEST_GOLD_TASK_2, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island,
                                                 mean_centering, unit_vectors, save_file_ranks=save_file,
                                                 save_file_binary=save_file_binary, one_minus=True, name="latin")
...
def run_german_default(task_1_dir, task_2_dir, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island,
                       emb_type, emb_dim, window, iter, mean_centering, unit_vectors):
    print('-' * 70)
    print('German')
    ...
    if temporal_referencing:
        corp1_emb_file = emb_type + '.skipgram.german_corpus.' + str(
            emb_dim) + '_window-' + str(window) + '_iter-' + str(
            iter) + '_min-count-5.vec'
        corp_1_emb = os.path.join(EMBEDDINGS_EXPORT_PATH, 'german_corpus_temporal',
                                  corp1_emb_file)
        corp_2_emb = corp_1_emb

    target_words = GERMAN_TEST_TARGET_WORDS
    rho, acc, bin_thld, min_neighb_cnt = compare(GERMAN_TEST_CORPUS_1, GERMAN_TEST_CORPUS_2, corp_1_emb, corp_2_emb, target_words, GERMAN_TEST_GOLD_TASK_1,
                                                 GERMAN_TEST_GOLD_TASK_2, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island,
                                                 mean_centering, unit_vectors, save_file_ranks=save_file,
                                                 save_file_binary=save_file_binary, one_minus=True, name="german")
...
def run_english_default(task_1_dir, task_2_dir, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island,
                        emb_type, emb_dim, window, iter, mean_centering, unit_vectors):
    print('-' * 70)
    print('English')
    ...
    if temporal_referencing:
        corp1_emb_file = emb_type + '.skipgram.english_corpus.' + str(
            emb_dim) + '_window-' + str(window) + '_iter-' + str(
            iter) + '_min-count-5.vec'
        corp_1_emb = os.path.join(EMBEDDINGS_EXPORT_PATH, 'english_corpus_temporal',
                                  corp1_emb_file)
        corp_2_emb = corp_1_emb

    target_words = ENGLISH_TEST_TARGET_WORDS
    rho, acc, bin_thld, min_neighb_cnt = compare(ENGLISH_TEST_CORPUS_1, ENGLISH_TEST_CORPUS_2, corp_1_emb, corp_2_emb, target_words, ENGLISH_TEST_GOLD_TASK_1,
                                                 ENGLISH_TEST_GOLD_TASK_2, reverse_emb, use_bin_thld, use_nearest_neigbh, use_island,
                                                 mean_centering, unit_vectors, save_file_ranks=save_file,
                                                 save_file_binary=save_file_binary, one_minus=True, name="english")
...
def compare(corpus1_path, corpus2_path, src_emb_path, trg_emb_path, target_words_path, gold_file_task1, gold_file_task2, reverse, use_binary_threshold,
            use_nearest_neigbhrs, use_island, mean_centering, unit_vectors,
            xform=None, max_links=100000, run_spearman=True, save_file_ranks=None, save_file_binary=None,
            one_minus=False, topn=100, name=""):
    """
    ...
    run_transform = False
    if xform is None and not temporal_referencing:
    ...
    c1_sents, _ = load_test_data(corpus1_path)
    c2_sents, _ = load_test_data(corpus2_path)

    island_dict = {}
    island_file_name = name + '_window-' + str(island_size) + '.json'
    island_file_path = os.path.join(ISLAND_DIR, island_file_name)
    if os.path.exists(island_file_path):
        island_dict = json.load(open(island_file_path))
    for target_word in tqdm(target_words):
        # print("Word:" + str(target_word), end='')
        sim, sim_to_orig_word, sim_to_trans_vec = compare_sense(target_word, src_emb, trg_emb, trans_matrix, temporal_referencing, topn)

        if use_island:
            # island_result.append(compare_sense_island(target_word, src_emb, trg_emb, trans_matrix, c1_sents, c2_sents, island_size, island_num))
            i_result, num_il_c1, num_il_c2, islands = compare_sense_island(target_word, src_emb, trg_emb, trans_matrix, c1_sents, c2_sents, island_size, island_num, island_dict)
            island_result.append(i_result)
            if target_word not in island_dict:
                island_dict[target_word] = {
                    'num_island_c1': num_il_c1,
                    'num_island_c2': num_il_c2,
                    'islands': islands
                }
        else:
            island_result.append(0)
    ...
    if not os.path.exists(island_file_path):
        with open(island_file_path, 'w') as outfile:
            json.dump(island_dict, outfile)

    plot_sim = copy.deepcopy(similarities_unchanged)
    plot_sim.sort()
    # plot_sim.sort()
    x = np.arange(len(plot_sim))

    fig = plt.figure()
    plt.plot(x, plot_sim, "o")
    plt.xlabel("word index")
    plt.ylabel("similarity score")
    fig.savefig(src_emb_path + '.png', dpi=200)
    ...
    if use_binary_threshold is True:
        ...
        # avg_sim = np.median(similarities_unchanged)
        temp = copy.deepcopy(similarities_unchanged)
        temp.sort()
        if name == "english":
            avg_sim = temp[17]
        if name == "german":
    ...
    if use_island:
        print("Finish computing islands")

    # iterate again over words and compute binary task
    for target_word, sim, sim_to_orig_word, sim_to_trans_vec, nearest_neigbh_size, island_bin in zip(
            target_words, similarities_unchanged, similarities_to_orig_word, similarities_to_trans_vec, neighbrs_inter_sizes, island_result):
        if use_binary_threshold is True:
        ...
        if use_island:
            binar_change.append(island_bin)
    ...
    with open(output_file_binary, 'w', encoding='utf-8') as f:
...
def load_target_words(target_words_path, load_labels=True):
    with open(target_words_path, 'r', encoding='utf-8', errors='ignore') as f:
        lines = f.readlines()
