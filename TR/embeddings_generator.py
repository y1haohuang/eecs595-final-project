...
from cca.config import EMBEDDINGS_EXPORT_PATH, ENGLISH_TEST_CORPUS_1, ENGLISH_TEST_CORPUS_2, \
    GERMAN_TEST_CORPUS_1, GERMAN_TEST_CORPUS_2, SWEDISH_TEST_CORPUS_1, SWEDISH_TEST_CORPUS_2, LATIN_TEST_CORPUS_1, \
    LATIN_TEST_CORPUS_2, ENGLISH_TEST_TARGET_WORDS, GERMAN_TEST_TARGET_WORDS, LATIN_TEST_TARGET_WORDS, \
    SWEDISH_TEST_TARGET_WORDS
...
def add_suffix(target_words, c, suffix):
    for i, sentence in enumerate(c):
...
def train_temporal_referencing(export_dir=EMBEDDINGS_EXPORT_PATH):
    datasets = {
        'english': [ENGLISH_TEST_CORPUS_1, ENGLISH_TEST_CORPUS_2, ENGLISH_TEST_TARGET_WORDS],
        'german': [GERMAN_TEST_CORPUS_1, GERMAN_TEST_CORPUS_2, GERMAN_TEST_TARGET_WORDS],
        'latin': [LATIN_TEST_CORPUS_1, LATIN_TEST_CORPUS_2, LATIN_TEST_TARGET_WORDS],
        'swedish': [SWEDISH_TEST_CORPUS_1, SWEDISH_TEST_CORPUS_2, SWEDISH_TEST_TARGET_WORDS]
    }

    for key, c_list in datasets.items():
        corpus_dir = os.path.join(export_dir, key + '_corpus_temporal')
        pathlib.Path(corpus_dir).mkdir(parents=True, exist_ok=True)

        _, target_words = load_target_words(c_list[2])

        c1, path = load_test_data(c_list[0])
        #         path here is useless
        c1 = add_suffix(target_words, c1, '_1')

        c2, path = load_test_data(c_list[1])
        c2 = add_suffix(target_words, c2, '_2')

        c = c1 + c2
        perform_train_and_save(None, key + '_corpus', c,
                               corpus_dir, path)
...
def load_target_words(target_words_path, load_labels=False):
